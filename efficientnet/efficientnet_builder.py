...
def get_model_params(model_name, override_params, data_format='channels_first'):
  """Get the block args and global params for a given model."""
  if model_name.startswith('efficientnet'):
    width_coefficient, depth_coefficient, _, dropout_rate = (
        efficientnet_params(model_name))
    blocks_args, global_params = efficientnet(
        width_coefficient, depth_coefficient, dropout_rate, data_format=data_format)
  else:
...
def build_model_base(images, model_name, training, reuse=False, feature_only=True, override_params=None, data_format='channels_first'):
  ...
  with tf.variable_scope(model_name, reuse=reuse):
    model = efficientnet_model.Model(blocks_args, global_params)
    features = model(images, training=training, features_only=feature_only)
